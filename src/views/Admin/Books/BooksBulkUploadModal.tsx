...
import { BooksValidationResponse, UploadMode } from 'src/store/types/bulk'
...
const BooksBulkUploadModal = ({ isOpen, onClose, onSuccess }: BooksBulkUploadModalProps) => {
  ...
  // API Mutations
  const [downloadTemplate, { isLoading: isDownloadingTemplate }] = useDownloadBooksTemplateMutation()
  const [validateBooks, { isLoading: isValidating }] = useValidateBooksMutation()
  const [uploadBooks, { isLoading: isUploading }] = useUploadBooksMutation()
  const [downloadErrorSheet, { isLoading: isDownloadingErrorSheet }] = useDownloadBooksErrorSheetMutation()

  // State
  const [includeOld, setIncludeOld] = useState<boolean>(false)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [validationResult, setValidationResult] = useState<BooksValidationResponse | null>(null)
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const fileName = includeOld ? 'books_inventory_mass_update.xlsx' : 'books_inventory_template.xlsx'
      downloadDocument(url, fileName)
      triggerToast('Books template downloaded successfully', { variant: ToastVariants.SUCCESS })
    } catch (err: any) {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      const res = await validateBooks(formData).unwrap()
      setValidationResult(res)
      if (res.error_count === 0) {
        triggerToast(`Validation passed! ${res.valid_count} book(s) ready for import.`, {
  ...
  const handleDownloadErrorSheet = async () => {
    ...
    try {
      ...
      const url = window.URL.createObjectURL(blob)
      downloadDocument(url, 'books_errors_to_fix.xlsx')
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      const res = await uploadBooks({ formData, mode: uploadMode }).unwrap()
      triggerToast(res.message || 'Books uploaded successfully!', { variant: ToastVariants.SUCCESS })
      onSuccess?.()
      handleClose()
    } catch (err: any) {
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, pt: 1 }}>
            ...
            >
              <Box>
                ...
                <Typography variant='body2' color='text.secondary'>
                  Download the two-sheet inventory and curriculum mapping template.
                </Typography>
              </Box>
        ...
        {isValidating && (
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2, py: 2 }}>
            ...
            <Typography variant='body2' color='text.secondary'>
              Checking Books_Master and Book_Mappings formulas...
            </Typography>
          </Box>
        )}
                ...
                <FormControlLabel
                  ...
                  label={
                    <Box>
                      ...
                      <Typography variant='caption' color='text.secondary'>
                        Imports the {validationResult.valid_count} valid book(s) and skips errored rows.
                      </Typography>
                    </Box>
                  }
                  sx={{ mr: 4 }}
        ...
        <ChaarvyFlex className={{ justifyContent: 'flex-end', alignItems: 'center', gap: 2 }}>
          ...
          >
            {validationResult ? `Confirm & Import (${validationResult.valid_count} Books)` : 'Confirm & Import'}
          </LoadingButton>
        </ChaarvyFlex>
      </Box>
    </ChaarvyModal>
