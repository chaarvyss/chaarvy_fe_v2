...
const FONT_FAMILIES = [
  ...
  'Trebuchet MS', 'Verdana', 'monospace', 'cursive', 'fantasy',
...
const DesignerPage = () => {
  ...
  const canvasWidth = pageSize === 'Custom'
    ? (orientation === 'portrait' ? customWidth : customHeight)
    : (orientation === 'portrait' ? PAGE_SIZES[pageSize].width : PAGE_SIZES[pageSize].height)
  const canvasHeight = pageSize === 'Custom'
    ? (orientation === 'portrait' ? customHeight : customWidth)
    : (orientation === 'portrait' ? PAGE_SIZES[pageSize].height : PAGE_SIZES[pageSize].width)
  ...
  const saveToHistory = useCallback((newPlaced: PlacedField[]) => {
    setHistory(prev => {
  ...
  const handleResizeMouseDown = useCallback((e: React.MouseEvent, item: PlacedField, handle: 'se' | 'sw' | 'ne' | 'nw' | 'n' | 's' | 'e' | 'w') => {
    e.stopPropagation()
    setResizeState({
      isResizing: true,
      itemId: item.id,
  ...
  const renderPlacedItem = (p: PlacedField) => {
    if (p.type === 'field') {
      ...
      const borderStyle = p.borderWidth ? `${p.borderWidth}px ${p.borderStyle || 'solid'} ${p.borderColor || '#333'}` : '2px solid #333'
  ...
  return (
    <div style={{ display: 'flex', gap: 20, padding: 20, position: 'relative' }}>
      ...
      <div style={{ width: showSidebar ? 240 : 60, maxHeight: '90vh', overflowY: 'auto', transition: 'width 0.3s ease', position: 'relative' }}>
        ...
        {availableItems
          ...
          .map(f => (
            <div
              ...
              style={{ padding: 8, border: '1px solid #ddd', marginBottom: 8, cursor: 'grab', background: '#f9f9f9', borderRadius: 4 }}
            >
              {f.label}
            </div>
          ))}
          ...
          {availableItems
            ...
            .map(f => (
              ...
              >
                ...
                {f.key === 'circle' && <div style={{ width: 18, height: 18, border: '2px solid #333', borderRadius: '50%' }} />}
        ...
        <div style={{ marginTop: 16 }}>
          ...
          {pageSize === 'Custom' && (
            <>
              <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Width (px)</label>
              <input
                type="number"
                value={customWidth}
                ...
                min="100"
              />
              <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Height (px)</label>
              <input
                type="number"
                value={customHeight}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, paddingTop: 50 }}>
            {/* Minimized icons */}
            <div title="Fields" style={{ fontSize: 20, cursor: 'pointer', padding: 8, borderRadius: 4, background: '#f9f9f9' }}>
              📝
            </div>
            <div title="Shapes" style={{ fontSize: 20, cursor: 'pointer', padding: 8, borderRadius: 4, background: '#f9f9f9' }}>
              ▢
            </div>
            <div title="Images" style={{ fontSize: 20, cursor: 'pointer', padding: 8, borderRadius: 4, background: '#f9f9f9' }}>
              🖼️
            </div>
            <div title="Upload" style={{ fontSize: 20, cursor: 'pointer', padding: 8, borderRadius: 4, background: '#f9f9f9' }}
              onClick={() => fileInputRef.current?.click()}
            >
              📁
            </div>
            <hr style={{ width: '80%', border: 'none', borderTop: '1px solid #ddd' }} />
            <div title="Undo" onClick={undo} style={{ fontSize: 20, cursor: historyIndex === 0 ? 'not-allowed' : 'pointer', padding: 8, borderRadius: 4, background: historyIndex === 0 ? '#e0e0e0' : '#f9f9f9', opacity: historyIndex === 0 ? 0.5 : 1 }}>
              ↶
            </div>
            <div title="Redo" onClick={redo} style={{ fontSize: 20, cursor: historyIndex >= history.length - 1 ? 'not-allowed' : 'pointer', padding: 8, borderRadius: 4, background: historyIndex >= history.length - 1 ? '#e0e0e0' : '#f9f9f9', opacity: historyIndex >= history.length - 1 ? 0.5 : 1 }}>
              ↷
            </div>
            <div title="Export" onClick={exportTemplate} style={{ fontSize: 20, cursor: 'pointer', padding: 8, borderRadius: 4, background: '#4CAF50', color: 'white' }}>
              💾
            </div>
            <div title="Import" onClick={() => document.getElementById('import-input')?.click()} style={{ fontSize: 20, cursor: 'pointer', padding: 8, borderRadius: 4, background: '#FF9800', color: 'white' }}>
              📂
            </div>
          </div>
        )}
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <h3 style={{ margin: 0 }}>Canvas ({pageSize === 'Custom' ? `Custom (${customWidth} × ${customHeight} px)` : PAGE_SIZES[pageSize].label} - {orientation === 'portrait' ? 'Portrait' : 'Landscape'})</h3>
        ...
        <div style={{ overflow: 'auto', maxHeight: 'calc(90vh - 80px)', border: '1px solid #e0e0e0', padding: 20, background: '#f5f5f5' }}>
        <div
          ref={canvasRef}
          onDrop={handleDrop}
        ...
        >
          {placed.sort((a, b) => (a.zIndex || 0) - (b.zIndex || 0)).map(p => (
            <div
              key={p.id}
              draggable={false}
        ...
        <div style={{ marginTop: 12 }}>
          <button onClick={() => window.open('/api/templates/preview?clientId=' + clientId)} style={{ padding: '8px 16px', cursor: 'pointer' }}>
        ...
        return (
          <div style={{ width: 280, maxHeight: '90vh', overflowY: 'auto', borderLeft: '1px solid #ddd', paddingLeft: 20 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Position X</label>
              <input
                type="number"
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Position Y</label>
              <input
                type="number"
            ...
            {(item.type === 'shape' || item.type === 'image' || item.type === 'text') && (
              <>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Width</label>
                  <input
                    type="number"
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Height</label>
                  <input
                    type="number"
            ...
            {(item.type === 'text' || item.type === 'field') && (
              <>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Font Size</label>
                  <input
                    type="number"
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Font Family</label>
                  <select
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Font Weight</label>
                  <select
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Color</label>
                  <input
                    type="color"
                    value={item.color || '#000000'}
                    onChange={e => updateItemProperty(item.id, 'color', e.target.value)}
                    style={{ width: '100%', height: 40, padding: 2, border: '1px solid #ddd', borderRadius: 4, cursor: 'pointer' }}
                  />
                </div>

                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Letter Spacing (px)</label>
                  <input
                    type="number"
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Text Alignment</label>
            ...
            {/* Rotation Control */}
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Rotation (degrees)</label>
              <input
                type="number"
                min="0"
                max="360"
            ...
            {(item.type === 'shape' || item.type === 'text' || item.type === 'field') && (
              <>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Border Width (px)</label>
                  <input
                    type="number"
                    min="0"
                ...
                {item.borderWidth && item.borderWidth > 0 && (
                  <>
                    <div style={{ marginBottom: 16 }}>
                      <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Border Color</label>
                      <input
                        type="color"
                    ...
                    <div style={{ marginBottom: 16 }}>
                      <label style={{ display: 'block', marginBottom: 4, fontSize: 12, fontWeight: 500, color: '#666' }}>Border Style</label>
                      <select
            ...
            {/* Z-Index Controls */}
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', marginBottom: 8, fontSize: 12, fontWeight: 500, color: '#666' }}>Layer Order</label>
